...
# CMake config dir for SPIRV-Headers — generated by build_android_ggml_gpu_backends.sh so that
# find_package(SPIRV-Headers CONFIG REQUIRED) in ggml-vulkan/CMakeLists.txt can find the config.
...
build_for_abi() {
  ...
  if [ "$ABI_SUPPORTS_GPU" = true ] && [ "$BUILD_VULKAN" = true ] && [ "$VULKAN_AVAILABLE" = true ]; then
    ...
    # Also pass the generated CMake config dir so find_package(SPIRV-Headers CONFIG REQUIRED)
    # in ggml-vulkan/CMakeLists.txt resolves without needing a system-installed cmake package.
    if [ -d "$SPIRV_STAGED_DIR/spirv" ]; then
      ABI_GPU_FLAGS+=(-DCMAKE_CXX_FLAGS="-Wno-deprecated-declarations -I${SPIRV_STAGED_DIR}")
      # Ensure SPIRV-HeadersConfig.cmake exists — it is normally created by
      # build_android_ggml_gpu_backends.sh, but regenerate here as a fallback.
      if [ ! -f "$SPIRV_CMAKE_DIR/SPIRV-HeadersConfig.cmake" ]; then
...
SPIRVCMAKE
      ...
      ABI_GPU_FLAGS+=(-DSPIRV-Headers_DIR="$SPIRV_CMAKE_DIR")

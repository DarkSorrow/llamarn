...
# Locate SPIRV-Headers on the system (installed via 'apt install spirv-headers' on Linux,
# or via Vulkan SDK on macOS/Windows). Per llama.cpp build.md recommendation.
detect_spirv_include() {
  local candidates=(
    "/usr/include"                           # Linux: apt install spirv-headers
    "/usr/local/include"                     # Homebrew / manual install
    "$VULKAN_SDK/include"                    # LunarG Vulkan SDK (if set)
  )
  for dir in "${candidates[@]}"; do
    if [ -f "$dir/spirv/unified1/spirv.hpp" ]; then
      echo "$dir"
      return 0
    fi
  done
...
if [ "$BUILD_VULKAN" = true ]; then
  ...
  SPIRV_INCLUDE_PATH=$(detect_spirv_include)
  if [ -z "$SPIRV_INCLUDE_PATH" ]; then
    echo -e "${RED}SPIRV-Headers not found. Install with:${NC}"
    echo -e "${YELLOW}  Linux:  sudo apt-get install spirv-headers${NC}"
    echo -e "${YELLOW}  macOS:  brew install spirv-headers${NC}"
    exit 1
  fi
  echo -e "${GREEN}SPIRV headers found at: $SPIRV_INCLUDE_PATH${NC}"
